# Character bible — LOCKED. Every prompt must reuse verbatim.
STYLE: full color Korean webtoon style, solo leveling manhwa, clean cel shading, vertical composition, modern characters
STYLE_NEG: no medieval armor, no fantasy castle drift, no photoreal, no chibi, no sketch — modern Shenzhen only
REN_QI: young Chinese male structural engineer, age 24, short black hair, grey hoodie and jeans, determined tired eyes
SEED_BASE: 777 (panel N uses 777+N, never random)
RULE: never change hair/clothes words. Only change shot + action + lighting.
