# Chapter 9: Harvesting the Divine (phase-009) — manhwa adaptation task for opencode

Read FIRST: `manhwa/bible/characters.md` (locked characters below), `AGENTS.md` (hard rules),
`workspace/phase-009/source.txt` (chapter text), `workspace/phase-009/meta.json`.
...
## Step 2 — write workspace/phase-009/panels.json
...
Run: `python3 scripts/render.py phase-009` then `python3 scripts/judge.py phase-009`.
If judge reports a panel needs retry, DO NOT just bump the seed and hope — use
`python3 scripts/render.py fix phase-009 <panel_id> "<specific correction, e.g.
'face doesn't match reference, fix hairstyle and jawline to match, keep pose'>"`
so the fix targets what's actually wrong.

DONE = `panels.json` has 8+ panels AND `out/phase-009/panelNN.jpg` all exist >10KB
