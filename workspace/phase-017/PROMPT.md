# Chapter 17: Public Enemy Number One (phase-017) — manhwa adaptation task for opencode

Read FIRST: `manhwa/bible/characters.md` (locked characters below), `AGENTS.md` (hard rules),
`workspace/phase-017/source.txt` (chapter text), `workspace/phase-017/meta.json`.
...
## Step 2 — write workspace/phase-017/panels.json
...
Run: `python3 scripts/render.py phase-017` then `python3 scripts/judge.py phase-017`.
If judge reports a panel needs retry, DO NOT just bump the seed and hope — use
`python3 scripts/render.py fix phase-017 <panel_id> "<specific correction, e.g.
'face doesn't match reference, fix hairstyle and jawline to match, keep pose'>"`
so the fix targets what's actually wrong.

DONE = `panels.json` has 8+ panels AND `out/phase-017/panelNN.jpg` all exist >10KB
