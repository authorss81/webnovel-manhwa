# Chapter 50: Pirates of the Acid Sea (phase-050) — manhwa adaptation task for opencode

Read FIRST: `manhwa/bible/characters.md` (locked characters below), `AGENTS.md` (hard rules),
`workspace/phase-050/source.txt` (chapter text), `workspace/phase-050/meta.json`.
...
## Step 2 — write workspace/phase-050/panels.json
...
Run: `python3 scripts/render.py phase-050` then `python3 scripts/judge.py phase-050`.
If judge reports a panel needs retry, DO NOT just bump the seed and hope — use
`python3 scripts/render.py fix phase-050 <panel_id> "<specific correction, e.g.
'face doesn't match reference, fix hairstyle and jawline to match, keep pose'>"`
so the fix targets what's actually wrong.

DONE = `panels.json` has 8+ panels AND `out/phase-050/panelNN.jpg` all exist >10KB
